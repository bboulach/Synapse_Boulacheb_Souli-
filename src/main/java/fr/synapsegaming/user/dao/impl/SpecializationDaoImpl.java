...
implements SpecializationDao {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public List<Specialization> listMostPlayed(int nbSpecialization) {
        Session session = this.getSession();
        try {
            Query query = session
                    .createQuery("from Specialization s order by size(s.users) desc");
            query.setMaxResults(nbSpecialization);
            return query.list();
        } catch (Exception e) {
            LOGGER.warn(e);
            LOGGER.warn("Pas de specialization saisie");
            return Collections.emptyList();
        } finally {
            session.close();
        }
